...
**Before dispatching any tasks, ensure an isolated workspace exists:**

- Use **sdlc:using-git-worktrees** to create a worktree and feature branch
...
digraph process {
    ...
    "Use sdlc:finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];
    ...
    "Dispatch final code reviewer subagent for entire implementation" -> "Use sdlc:finishing-a-development-branch";
}
```
...
[Read plan file once: .sdlc/SPEC<NNN>-<module>-<title>/plan.md]
...
You: "User level (~/.config/sdlc/hooks/)"
...
- **sdlc:using-git-worktrees** - REQUIRED: Set up isolated workspace before
  starting
- **sdlc:writing-plans** - Creates the plan this skill executes
- **sdlc:requesting-code-review** - Code review template for reviewer subagents
- **sdlc:finishing-a-development-branch** - Complete development after all tasks

**Subagents should use:**

- **sdlc:test-driven-development** - Subagents follow TDD for each task

**Alternative workflow:**

- **sdlc:executing-plans** - Use for parallel session instead of same-session
  execution
