...
Deno.test({
  ...
  async fn(t) {
    ...
    await t.step("POST /mcp without session — auth runs first (no Authorization → 401)", async () => {
    ...
    await t.step("GET /mcp without session ID header — returns 400", async () => {
      const res = await mcpApp.fetchClient("/mcp", {
        method: "GET",
    ...
    await t.step("/health endpoint — returns 200 with server status", async () => {
    ...
    await t.step("health_list — returns 200 with data", async () => {
      const res = await callTool(mcpApp.fetchClient, "health_list", { limit: 10 }, sessionId, mcpApp.apiKey);
    ...
    await t.step("health_create — creates record and returns 200", async () => {
      const res = await callTool(
        mcpApp.fetchClient, "health_create",
        { data: { e2eTest: true, score: 95 } },
        sessionId, mcpApp.apiKey,
      );
      assertEquals(res.status === 200 || res.status === 202, true);
      assertExists((res.data as any).result);
    });
    ...
    await t.step("health_get — retrieves existing record by id", async () => {
      ...
      const res = await callTool(mcpApp.fetchClient, "health_get", { id }, sessionId, mcpApp.apiKey);
      assertEquals(res.status === 200 || res.status === 202, true);
      assertEquals((res.data as any).jsonrpc, "2.0");
    });

    await t.step("health_get — handles not-found record gracefully", async () => {
      const res = await callTool(mcpApp.fetchClient, "health_get", { id: 999999 }, sessionId, mcpApp.apiKey);
      assertEquals(res.status === 200 || res.status === 202, true);
      assertEquals((res.data as any).jsonrpc, "2.0");
    });

    await t.step("health_update — updates existing record and returns 200", async () => {
      const res = await callTool(
        mcpApp.fetchClient, "health_update",
    ...
    await t.step("health_update — returns 200 for non-existent record", async () => {
      const res = await callTool(
        mcpApp.fetchClient, "health_update",
    ...
    await t.step("health_delete — deletes record and returns 200", async () => {
      ...
      const res = await callTool(mcpApp.fetchClient, "health_delete", { id }, sessionId, mcpApp.apiKey);
      assertEquals(res.status === 200 || res.status === 202, true);
    });

    await t.step("health_delete — returns 200 for non-existent record", async () => {
      const res = await callTool(mcpApp.fetchClient, "health_delete", { id: 999997 }, sessionId, mcpApp.apiKey);
      assertEquals(res.status === 200 || res.status === 202, true);
    });

    await t.step("health_gap_test — returns 200 for existing record", async () => {
      const res = await callTool(mcpApp.fetchClient, "health_gap_test", { id: 1 }, sessionId, mcpApp.apiKey);
      assertEquals(res.status === 200 || res.status === 202, true);
    });

    // ── Resource steps ──────────────────────────────────────────────────────────

    await t.step("resources/list — returns health schema resource", async () => {
      const res = await mcpApp.fetchClient("/mcp", {
        method: "POST",
      ...
      assertExists(schemaResource, "Resource geotools://health/schema should exist");
