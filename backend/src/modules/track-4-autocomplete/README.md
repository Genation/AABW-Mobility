...
```bash
# Basic autocomplete
curl "http://localhost:8000/api/v1/track-4/suggest?q=cafe"

# With location for regional ranking
curl "http://localhost:8000/api/v1/track-4/suggest?q=atm&lat=10.7759&lng=106.7031&limit=5"

# Typo handling (fuzzy match)
curl "http://localhost:8000/api/v1/track-4/suggest?q=nguyen+huee"

# Abbreviation expansion (ks → khách sạn)
curl "http://localhost:8000/api/v1/track-4/suggest?q=ks+da+nang"

# Accent-less Vietnamese
curl "http://localhost:8000/api/v1/track-4/suggest?q=benh+vien"

# Mixed English-Vietnamese
curl "http://localhost:8000/api/v1/track-4/suggest?q=coffee+near"
```
